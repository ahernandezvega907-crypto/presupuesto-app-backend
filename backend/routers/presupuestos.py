...
def crear_presupuesto(
    ...
    nuevo = PresupuestoMensual(
        limite=p.limite, mes=p.mes, ano=p.year,
...
def estado_presupuestos(
    ...
    for p in presupuestos:
        gastado = db.query(func.sum(Transaccion.monto)).filter(
            ...
            func.strftime("%Y", Transaccion.fecha) == str(p.ano)
        ).scalar() or 0
        ...
        resultado.append({
            ...
            "alerta": "ALERTA: Limite superado" if gastado > p.limite else "Dentro del presupuesto"
        })
    return resultado